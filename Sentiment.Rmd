...
The general feeling of the speeches, broken down by sentiments, for each president were explored using both the *nrc* and *bing* lexicons. The neutral words were excluded, as they dominate over the other sentiments.

```{r Sentiment Prop by President, include = FALSE, message=FALSE, echo=FALSE, fig.align='center', fig.show="hold", out.width="60%", fig.cap="Proportion of positive to negative words by president based on the *bing* lexicon"}
...
```{r Sentiment Bars by President (nrc), include = FALSE, message=FALSE, echo=FALSE, fig.align='center', fig.show="hold", out.width="60%", fig.cap="Proportion of positive to negative words by president based on the *bing* lexicon"}
sona_sentiment %>%
...
It can be seen that, besides *negative* and *positive* sentiments, common themes were *trust* and *anticipation*. To gain some insight on which words specifically were associated with these two themes, the top 2 words associated with *trust* or *anticipation* for each president are displayed in a table below.
...
Changes of the sentiments over time were investigated. Since there is a large number of neutral words, these were once again excluded from the analysis. Line plots with corresponding smoothed lines were created to assess whether there were any shifts in positive or negative sentiments.

```{r Trends over Time, include = FALSE, message=FALSE, echo=FALSE, fig.align='center', fig.show="hold", out.width="60%", fig.cap="Proportion of positive to negative words by president based on the *bing* lexicon"}
...
From the plot, it seems that there has generally been a decline in negative sentiment over the years. To investigate whether this change is statistically significant, a logistic regression model is fit. Logistic regression is a modelling technique, based on regression, in which the dependent variable is binary. This is an appropriate model to use because the dependent variable has a binary outcome: *positive* or *negative*.

```{r Fit Binomial GLM, include = FALSE, message=FALSE, echo=FALSE}
...
The p-values obtained from the analysis are above the traditional threshold of 0.05, meaning there was no significant decrease in the proportion of negative sentiments over time.

### Bigrams
An n-gram is a sequence of *n* words in a text. Bigrams are pairs of words that are adjacent to each other. Some words, such as "not" change the meaning of the preceding word by negating it. It is therefore important to consider this when performing sentiment analysis. The following section will explore the most common positive and negative bigrams in the speeches using the *bing* dictionary. To determine the net sentiment of a bigram, positive words are given a value of 1 and negative words are given a value of -1. Words that are preceded by a negation word ('not', 'no', 'never', 'without' or 'anti') had their sentiment reversed.
A few of the words in the *bing* dictionary did not agree with the context and were thus adjusted in the analysis. The words 'honour' and 'hope' were changed from *neutral* to *positive*. The word 'oppression' was changed from *neutral* to *negative*.

```{r Dealing With Negation, include = FALSE, message=FALSE, echo=FALSE}
...
```{r Top 20 Positive Bigrams, include = FALSE, message=FALSE, echo=FALSE}
bigrams_separated %>%
...
```{r Top 20 Negative Bigrams, include = FALSE, message=FALSE, echo=FALSE}
bigrams_separated %>%
...
```{r Top 20 Negative Bigrams with Negation, include = FALSE, message=FALSE, echo=FALSE}
bigrams_separated %>%
  filter(net_sentiment < 0) %>% # get negative bigrams
